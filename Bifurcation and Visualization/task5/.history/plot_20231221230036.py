...
def simulation_sir(b,random_state):
    t_0 = 0  # Initial time
    t_end = 100000  # End time for simulation
...
def sir_trajectory(SIM0_list,show_traj,para,b=0.01,mu1=10.45):
    ...
    if para ==  'mu1':
       ...
       ax.set_ylim([0, 0.25])
       ax.set_zlim([0, 8])
       para =  'backward bifurcation: mu1'
    if para ==  'b':
       ax.set_xlim([193, 200])
       ax.set_ylim([0, 0.08])
       ax.set_zlim([0, 7])
       para= 'Hopf bifurcation: y0='+str(SIM0_list[0])+' b'
       bif_para = b
      # choose if show trajectory, save fig
    if show_traj == False:
      ax.set_title('Initial and final points for '+para+'='+str(mu1)) 
      fig.tight_layout()      
      plt.savefig('Initial and final points for '+para+'='+str(mu1)+'.png')
    if show_traj == True:
      ax.scatter(sol.y[0], sol.y[1], sol.y[2],s=3, c='b',zorder=0,alpha=0.1)
      ax.set_title('SIR trajectory for '+para+'='+str(bif_para)) 
      fig.tight_layout()
      plt.savefig('SIR trajectory for '+para+'='+str(bif_para)+'.png')
    plt.show()