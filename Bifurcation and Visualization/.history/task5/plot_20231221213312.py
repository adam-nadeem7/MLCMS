...
def sir_trajectory(SIM0_list,show_traj,para,b=0.01,mu1=10.45):
    ...
    if show_traj == False:
      ax.set_title('Initial and final points for backward bifurcation: mu1='+str(mu1)) 
      fig.tight_layout()      
      plt.savefig('Initial and final points for backward bifurcation: mu1='+str(mu1)+'.png')
    if show_traj == True:
      ax.scatter(sol.y[0], sol.y[1], sol.y[2],s=3, c='b',zorder=0,alpha=np.linspace(0.001,0.05,t_end))
      ax.set_title('SIR trajectory for backward bifurcation: '+para+'='+str(bif_para)) 
      fig.tight_layout()
